...
async function makeApiCall(sortBy_Index=4,string = ""){
    ...
    if(string!=="") {
        ...
        if(sortBy_Index<4){
            if(sortBy_Index === 0 || sortBy_Index ===1) sortMoviesByRating(sortBy_Index,newMovies);
            if(sortBy_Index === 2 || sortBy_Index ===3) sortMoviesByRelease(sortBy_Index,newMovies);
        }
        else createMovieCards(newMovies);
    }   
}
